...
export function CreateStep2Status(props) {
    ...
    fetchy(process.env.NODE_ENV === 'development' ? 'http://localhost:18080/api/User/CreateStep2' : 'https://www.danspizza.dev/api/User/CreateStep2', 'POST', addies, headers)
        .catch((error) => {
...
export function CreateStep2Modal(props) {
    ...
    const handleFirstNameChange = (e) => {
        const validChar = charRegex.test(e.target.value)
        const validCharNum = e.target.value.length <= 24
        setValidFirstName(validChar && validCharNum)
        console.log('firstname validchar: ' + validChar + ' validcharnum: ' + validCharNum)
    ...
    const handleLastNameChange = (e) => {
        const validChar = charRegex.test(e.target.value)
        const validCharNum = e.target.value.length <= 50
        setValidLastName(validChar && validCharNum)
        console.log('lastName validchar: ' + validChar + ' validcharnum: ' + validCharNum)
        if (validChar && validCharNum) {
